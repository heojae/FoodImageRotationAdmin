...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        loss, targets_divided_to_each_cuda = criterion(outputs, targets)

        acc1 = 0
        for j in range(0, len(outputs)):
            output = outputs[j]
            target_divided_to_each_cuda = targets_divided_to_each_cuda[j]
            temp_acc1 = accuracy(output, target_divided_to_each_cuda, topk=(1,))
            acc1 += temp_acc1[0].item()
        acc1 /= len(outputs)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resume', default='', type=str, metavar='PATH',
                        # ./weight/data_parallel_separate_b0_bs_96.pth.tar
