...
from proto import inference_pb2, inference_pb2_grpc, empty_pb2
...
class InferenceFoodImageServicer(inference_pb2_grpc.InferenceFoodImageServicer):

    def Inference(self, request: inference_pb2.FoodB64Image,
                  context: grpc.aio.ServicerContext) -> inference_pb2.InferenceResult:
        global model
        print("inference")

        try:
            image: PIL.Image.Image = convert_b64image2pil_image(b64image=request.b64image)
    ...
    def LoadModel(self, request: inference_pb2.ModelPath, context: grpc.aio.ServicerContext) -> Empty:
        ...
        try:
            ...
            msg = "Invalid model path has come"
...
def serve() -> None:
    ...
    inference_pb2_grpc.add_InferenceFoodImageServicer_to_server(InferenceFoodImageServicer(), server)
